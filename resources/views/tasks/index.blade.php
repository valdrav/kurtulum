...
<div class="card hide-mobile">
    <div class="table-responsive">
        <table class="table table-vcenter card-table table-modern">
            <thead><tr><th>{{ __('tasks.title') }}</th><th>{{ __('tasks.description') }}</th><th>Öncelik</th><th>Durum</th><th>Termin</th><th>Atanan</th><th></th></tr></thead>
            <tbody>
                @forelse($tasks as $t)
                <tr>
                    <td>
        ...
        <div class="modal-content">
            <form method="POST" action="{{ route('tasks.update', $t) }}">@csrf @method('PUT')
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-2">
                        <div class="col-md-4"><label class="form-label">{{ __('tasks.priority') }}</label><select name="priority" class="form-select">@foreach(['low','medium','high','urgent'] as $p)<option value="{{ $p }}" @selected($t->priority===$p)>{{ $p }}</option>@endforeach</select></div>
                        <div class="col-md-4"><label class="form-label">{{ __('app.status') }}</label><select name="status" class="form-select">@foreach(['pending','in_progress','completed','cancelled'] as $s)<option value="{{ $s }}" @selected($t->status===$s)>{{ $s }}</option>@endforeach</select></div>
