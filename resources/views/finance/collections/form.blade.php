...
<div class="card" style="max-width:640px">
    <div class="card-body">
        <p class="text-muted small">{{ __('finance.edit_collection_hint') }}</p>
        <form method="POST" action="{{ route('finance.collections.update', $collection) }}">
            ...
            <div class="mb-3">
                ...
                <select name="account_id" class="form-select" required>
                    ...
                    <option value="{{ $a->id }}" @selected(old('account_id', $collection->account_id) == $a->id)>{{ $a->name }} ({{ $a->currency }})</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
            ...
            <div class="row g-2 mb-3">
                <div class="col-md-6">
                    ...
                    <input type="number" step="0.01" name="amount" class="form-control" value="{{ old('amount', $collection->amount) }}" required>
                </div>
                <div class="col-md-6">
                    <label class="form-label">{{ __('app.currency') }} *</label>
                    <select name="currency" class="form-select" required>
                        @foreach(registry()->currencyCodes() as $c)
                        <option value="{{ $c }}" @selected(old('currency', $collection->currency) === $c)>{{ $c }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="mb-3">
                ...
                <input type="number" step="0.000001" name="exchange_rate" class="form-control" value="{{ old('exchange_rate', $collection->exchange_rate) }}">
                <div class="form-hint">{{ __('finance.locked_rate_note') }}: {{ number_format((float) $collection->exchange_rate, 4, ',', '.') }}</div>
            </div>
            <div class="mb-3">
                <label class="form-label">{{ __('extensions.payment_method') }} *</label>
                <select name="payment_method_id" class="form-select" required>
                    @foreach($paymentMethods as $m)
                    <option value="{{ $m->id }}" @selected(old('payment_method_id', $collection->payment_method_id) == $m->id)>{{ $m->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
            ...
            <a href="{{ route('finance.accounts.show', $collection->account_id) }}" class="btn btn-ghost-secondary">{{ __('app.cancel') }}</a>
        </form>
    </div>
</div>
@endsection
