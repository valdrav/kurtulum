...
class RepairTreasuryBalancesCommand extends Command
{
    protected $signature = 'finance:repair-treasury-balances
                            {--account= : Belirli banka hesabı ID veya UUID}';

    protected $description = 'Recalculate company treasury balances from opening balance + ledger transactions';

    public function handle(): int
    {
        $query = Account::query()->companyTreasury();
        ...
        $this->info($fixed > 0 ? "{$fixed} banka hesabı bakiyesi güncellendi." : 'Tüm banka bakiyeleri tutarlı.');
